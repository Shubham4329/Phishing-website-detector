...
class FeatureExtraction:
    ...
    def web_traffic(self, url):
        try:
            ...
            if rank_tag is not None and 'RANK' in rank_tag.attrs:
                rank = int(rank_tag['RANK'])
                if rank < 100000:
                    return 0  # legitimate
                else:
                    return 2  # suspicious
            else:
                return 2  # suspicious (no rank information)
        except (urllib.error.URLError, urllib.error.HTTPError, TypeError) as e:
            # If there's an error fetching the data, consider it suspicious
            return 2  # suspicious
...
def getAttributes(url):
    ...
    # Drop any additional features not used in the model
    data_df = data_df.drop(['Having_IP', 'Redirection_//_symbol', 'Sub_domains'], axis=1, errors='ignore')
